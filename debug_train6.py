...
# Target depends on variables 0, 1, 2
...
for epoch in range(200):
    ...
    # ALWAYS observe the 3 relevant variables + random others
    M_O = torch.zeros(B, n_vars)
    for i in range(B):
        # Always include vars 0, 1, 2
        obs_idx = [0, 1, 2]
        # Add 27 random others
        other_vars = list(range(3, n_vars))
        extra = torch.randperm(len(other_vars))[:27].tolist()
        obs_idx += [other_vars[j] for j in extra]
        M_O[i, obs_idx] = 1.0
    x_sparse = x * M_O
